...
<body>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const d16Input = document.querySelector('input[name="d16"]');

            // Daftar g_checkbox yang perlu dikontrol
            const gCheckboxes = {
                b: document.getElementById('g_checkbox_b'),
                e: document.getElementById('g_checkbox_e'),
                g: document.getElementById('g_checkbox_g'),
                i: document.getElementById('g_checkbox_i'),
                j: document.getElementById('g_checkbox_j'),
                k: document.getElementById('g_checkbox_k')
            };

            // Fungsi untuk update realtime
            function updateF21Logic() {
                // Hanya satu checkbox yang bisa aktif
                const checkboxes = [f21CheckboxA, f21CheckboxB, f21CheckboxC];

                // Mutual exclusivity
                checkboxes.forEach(checkbox => {
                    checkbox.addEventListener('change', () => {
                        if (checkbox.checked) {
                            checkboxes.filter(c => c !== checkbox).forEach(c => c.checked = false);
                        }
                        applyGCheckboxRules();
                        updateF21Value();
                        saveToDatabase(); // Simpan perubahan ke database
                    });
                });

                // Update nilai f21 saat d16 berubah
                d16Input.addEventListener('input', () => {
                    updateF21Value();
                    saveToDatabase(); // Simpan perubahan ke database
                });
            }

            // Fungsi untuk update nilai f21
            function updateF21Value() {
                if (f21CheckboxA.checked) {
                    const d16Value = parseFloat(d16Input.value.replace(/\./g, '')) || 0;
                    f21Input.value = (d16Value / 2).toLocaleString('id-ID');
                } else {
                    f21Input.value = '0';
                }
            }

            // Fungsi untuk aturan g_checkbox
            function applyGCheckboxRules() {
                // Reset semua g_checkbox
                Object.values(gCheckboxes).forEach(checkbox => {
                    checkbox.disabled = false;
                    checkbox.checked = false;
                });

                if (f21CheckboxA.checked) {
                    // Nonaktifkan dan uncheck
                    ['b', 'e', 'g', 'i', 'j', 'k'].forEach(key => {
                        gCheckboxes[key].disabled = true;
                        gCheckboxes[key].checked = false;
                    });
                } else if (f21CheckboxB.checked) {
                    // Check otomatis g_checkbox_j
                    gCheckboxes.j.checked = true;

                    // Nonaktifkan lainnya
                    ['b', 'e', 'g', 'i', 'k'].forEach(key => {
                        gCheckboxes[key].disabled = true;
                        gCheckboxes[key].checked = false;
                    });
                } else if (f21CheckboxC.checked) {
                    // Check otomatis g_checkbox_g
                    gCheckboxes.g.checked = true;

                    // Nonaktifkan lainnya
                    ['b', 'e', 'i', 'j', 'k'].forEach(key => {
                        gCheckboxes[key].disabled = true;
                        gCheckboxes[key].checked = false;
                    });
                }
            ...
            function saveToDatabase() {
                ...
                formData.append('f21', f21Input.value);
                formData.append('f21_checkbox_a', f21CheckboxA.checked ? 'on' : 'off');
                formData.append('f21_checkbox_b', f21CheckboxB.checked ? 'on' : 'off');
                formData.append('f21_checkbox_c', f21CheckboxC.checked ? 'on' : 'off');

                // Kirim data ke server menggunakan AJAX
                fetch('<?= site_url('form/update_f21') ?>', {
                        method: 'POST',
                        body: formData
                    })
            ...
            // Tambahan untuk handle perubahan manual pada g_checkbox
            Object.values(gCheckboxes).forEach(checkbox => {
                checkbox.addEventListener('change', () => {
